...
namespace cred_system_back_end_app.Infrastructure.Data.ContextEntity
{
    public class DbContextEntity : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            DelegatesRelationshipConfiguration(modelBuilder);
            ProviderInsurerCompanyStatusRelConf(modelBuilder);
        }
        ...
        private void OneToOneRelationshipConfiguration(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<CredFormEntity>()
            .HasOne(cf => cf.CredFormStatusType);
        ...
        private void ProviderInsurerCompanyStatusRelConf(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<ProviderInsurerCompanyStatusEntity>()
                .HasOne(pics => pics.InsurerCompany)
                .WithMany(ic => ic.ProviderInsurerCompanyStatus)
                .HasForeignKey(pics => pics.InsurerCompanyId);
        }

        private void DelegatesRelationshipConfiguration(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<DelegateEntity>()
            .HasOne(d => d.DelegateCompany)
            .WithMany(dc => dc.Delegate)
            .HasForeignKey(d => d.DelegateCompanyId);

            modelBuilder.Entity<DelegateEntity>()
            .HasOne(d => d.DelegateType)
            .WithMany(dt => dt.Delegate)
            .HasForeignKey(d => d.DelegateTypeId);

            modelBuilder.Entity<DelegateEntity>()
            .HasMany(d => d.ProviderDelegate)
            .WithOne(pd => pd.Delegate)
